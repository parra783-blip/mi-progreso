...
function bindUI(){
  $$('.nav-item').forEach(b=>b.onclick=()=>{$$('.nav-item').forEach(x=>x.classList.remove('active'));b.classList.add('active');$$('.view').forEach(v=>v.classList.remove('active'));$('#'+b.dataset.view).classList.add('active')});
  ...
  $('#themeToggle').onclick=()=>{data.theme=data.theme==='dark'?'light':'dark';save();applyTheme()};
...
function renderAll(){renderHome();renderFood();renderWeight();renderDose();applyTheme()}
function renderHome(){
  ...
  const ws=[...data.weights].sort((a,b)=>a.date.localeCompare(b.date));const cur=ws.at(-1),first=ws[0];
  ...
  const ds=[...data.doses].sort((a,b)=>a.datetime.localeCompare(b.datetime));const last=ds.at(-1);
...
function renderAchievements(){
  const ws=[...data.weights].sort((a,b)=>a.date.localeCompare(b.date));const lost=ws.length>1?ws[0].weight-ws.at(-1).weight:0;
...
function renderWeight(){
  const arr=[...data.weights].sort((a,b)=>b.date.localeCompare(a.date));const asc=[...arr].reverse(),cur=asc.at(-1),first=asc[0],goal=+data.profile.goalWeight||null;
  $('#goalWeightLabel').textContent=goal?goal.toFixed(1)+' kg':'—';$('#goalProgressLabel').textContent=goal&&cur?Math.max(0,cur.weight-goal).toFixed(1)+' kg por perder':'Configúrala';
  $('#totalLost').textContent=cur&&first?(first.weight-cur.weight).toFixed(1)+' kg':'—';
...
function drawLine(canvas,arr){
  if(!canvas)return;const ctx=canvas.getContext('2d'),dpr=devicePixelRatio||1,w=canvas.clientWidth,h=canvas.height;canvas.width=w*dpr;canvas.height=h*dpr;ctx.scale(dpr,dpr);ctx.clearRect(0,0,w,h);
...
function exportCsv(){
  const rows=[['tipo','fecha','detalle','valor']];
  data.foods.forEach(x=>rows.push(['comida',x.date,`${x.meal}: ${x.name}`,`${x.calories} kcal`]));
  data.weights.forEach(x=>rows.push(['peso',x.date,'Peso',`${x.weight} kg`]));
  data.doses.forEach(x=>rows.push(['dosis',x.datetime,x.site,`${x.dose} mg`]));
...
function clearData(){if(confirm('¿Borrar definitivamente todos los registros de este dispositivo?')){data=structuredClone(defaultData);save()}}
